...
void loop() {
  ...
  // Update the LED states, incorporating the flash logic
  for (int i = 0; i < 16; i++) {
    // If the LED is in flash-off state and flash duration has passed, turn it back on
    if (ledFlashing[i] && millis() - ledFlashTimers[i] >= flashDuration) {
      ledFlashing[i] = false;  // Reset flashing state
      regWrite(i, buttonStates[i] ? HIGH : LOW);  // Turn the LED back on
    } else if (!ledFlashing[i]) {
      // Set the LED based on the button state if it's not flashing
      regWrite(i, buttonStates[i] ? HIGH : LOW);
    }
  }

  // Update active notes for MIDI (buttons 0-11 only)
  updateActiveNotes();

  // Handle note cycling if function2 is active
  if (function2State) {
    cycleNotes();
    adjustBPM();
  } else {
...
void cycleNotes() {
  cycleInterval = 60000 / bpm;  // Calculate the interval based on BPM

  if (millis() - lastNoteChangeTime >= cycleInterval) {
    if (activeNotesCount > 0) {  // Ensure there are active notes to play

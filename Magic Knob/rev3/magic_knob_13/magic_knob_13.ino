...
unsigned long lastClockSendTime = 0;
bool midiClockRunning = false;
const int MIDI_CLOCKS_PER_BEAT = 24; // Standard MIDI clock sends 24 pulses per quarter note
...
// Add this function to your code
void sendMidiClock() {
  // MIDI clock message calculations
  // At 120 BPM: One beat = 500ms, so one clock = 500ms/24 = 20.83ms
  unsigned long clockInterval = 60000 / (bpm * MIDI_CLOCKS_PER_BEAT);
  unsigned long currentTime = millis();
  
  // Check if it's time to send a clock pulse
  if (currentTime - lastClockSendTime >= clockInterval) {
    lastClockSendTime = currentTime;
...
void startMidiClock() {
  if (!midiClockRunning) {
    ...
    // Reset timer
    lastClockSendTime = millis();
    
    Serial.println("MIDI Clock Started");
  }
}

// Function to stop MIDI clock
void stopMidiClock() {
  if (midiClockRunning) {
...
// Modify your toggleFunction to start/stop MIDI clock when arpeggiator is toggled
void toggleFunction(int buttonIndex, bool isPressed) {
  ...
  if (buttonIndex == 15 && isPressed) { // Detect press event only
    ...
    // Start or stop MIDI clock based on arpeggiator state
    if (arp_state) {
      startMidiClock();
    } else {
      stopMidiClock();
...
// Modify your processEncoder function to also restart the MIDI clock when BPM changes
void processEncoder() {
  ...
  if (encoderPos != lastEncoderPos) {
    ...
    if (abs(accumulatedSteps) >= 4) {
      ...
      if (arp_state) {
        ...
        // If BPM changed and clock is running, restart the clock
        if (oldBpm != bpm && midiClockRunning) {
          // Send MIDI continue message (0xFB) to smoothly adjust tempo
...
void processMidiInput() {
  ...
  // Process all available MIDI messages in the buffer
  while (true) {
    rx = MidiUSB.read();
    if (rx.header == 0) break; // No more messages in the buffer
    
    // Process note on/off messages
    if ((rx.header == 0x09 || rx.header == 0x08) && rx.byte1 >= 0x90 && rx.byte1 <= 0x9F) {
      uint8_t receivedNote = rx.byte2;
      uint8_t velocity = rx.byte3;
      bool isNoteOn = rx.header == 0x09 && velocity > 0;
      
      // Print received note information
      Serial.print("Received MIDI: ");
      Serial.print(isNoteOn ? "Note On " : "Note Off ");
      Serial.print(receivedNote);
      Serial.print(" Velocity: ");
      Serial.println(velocity);
      
      // If this is the first note received, clear all currently selected notes
      if (isNoteOn && !noteStateChanged) {
        // Clear all current note selections
        for (int i = 0; i < 12; i++) {
      ...
      if (isNoteOn) {
        // Shift the received note to the current octave range and activate it
        uint8_t noteIndex = receivedNote % 12; // Get the note within the octave
        if (noteIndex >= 0 && noteIndex < 12) {
          buttonStates[noteIndex] = true;
          ledCurrentState[noteIndex] = true;
          regWrite(noteIndex, HIGH);
        }
      }
    }
  }
  
  // If notes were updated, update the active notes array
  if (noteStateChanged) {
    updateActiveNotes();
  }
}

// Finally, add this line to your loop() function after "if (arp_state) {"
...
void loop() {
  ...
  // Check for incoming MIDI messages - also high priority
  processMidiInput();
  ...
  // Process arpeggiator and MIDI clock (if active)
  if (arp_state) {
    cycleNotes();
    if (midiClockRunning) {
      sendMidiClock();

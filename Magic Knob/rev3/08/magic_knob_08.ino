...
const float maxInertia = 50.0;  // Adjust this value to change the maximum inertia
...
const float minInertiaThreshold = 0.05; // Minimum inertia to continue movement
unsigned long lastInertiaUpdateTime = 0;
const unsigned long INERTIA_UPDATE_INTERVAL = 15; // Update inertia effect every 20ms
const float inertiaPerDetent = 0.0125; // Adjust this value to change how much inertia is added per click
  ...
  if (encoderPos != lastEncoderPos) {
    ...
    if (abs(accumulatedSteps) >= 4) {
      ...
      if (arp_state) {
        ...
        if (knobInertia > maxInertia) knobInertia = maxInertia;
        if (knobInertia < -maxInertia) knobInertia = -maxInertia;
...
// Add this new function to handle the inertia effect
void processInertia() {
  ...
  if (currentTime - lastInertiaUpdateTime >= INERTIA_UPDATE_INTERVAL) {
    ...
    if (inertia_mode && abs(knobInertia) > minInertiaThreshold) {
      ...
      if (abs(knobInertia) <= minInertiaThreshold) {
        knobInertia = 0.0;
        return;
      }

...
void updatePotentiometers() {
  // Map potentiometer readings (0-1023) to a scale of 0.25 - 1.0
  kickScale = map(analogRead(kickPot), 0, 1023, 25, 100) / 100.0;
  snareScale = map(analogRead(snarePot), 0, 1023, 25, 100) / 100.0;
  hhScale = map(analogRead(hhPot), 0, 1023, 25, 100) / 100.0;
  crashScale = map(analogRead(crashPot), 0, 1023, 25, 100) / 100.0;
}
